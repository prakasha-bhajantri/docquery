...
# --- 0. CUSTOM CSS INJECTION (FINAL WORKING VERSION) ---
def inject_custom_css():
    st.markdown("""
        ...
        /* ==== USER MESSAGES → RIGHT (NOW 100% WORKS) ==== */
...
# 1. Main Title
# st.title("DocQuery: Chat with your Documents | RAG DEMO") # Already added this in CSS
...
for message in st.session_state.messages:
    # Use the role to determine alignment
...
if prompt := st.chat_input(f"Ask about: {st.session_state.selected_doc if st.session_state.selected_doc else 'No Document Selected'}..."):
    ...
    # 3. Assistant Message (Left side)
    with st.chat_message("assistant"):
        st.markdown(response_text)
    st.session_state.messages.append({"role": "assistant", "content": response_text})
    
    # Final Rerun to ensure state updates and CSS is rendered correctly
    st.rerun()